...
RG_WORKLOAD=$(render_name "$RG_PATTERN" "${RG_LIST[1]}")
RG_NETWORK=$(render_name "$RG_PATTERN" "${RG_LIST[0]}")
STORAGE_ACCOUNT_NAME=$(render_name "$STORAGE_ACCOUNT_PATTERN" "workload" "02")
STORAGE_ACCOUNT_NAME=${STORAGE_ACCOUNT_NAME//-/} # Remove hyphens
echo -e "${GREEN}Deploying Storage account:${NC} $STORAGE_ACCOUNT_NAME"
if [[ "$ACTION" == "apply" ]]; then
    # Create storage account if it doesn't exist
    az storage account create \
        --name "$STORAGE_ACCOUNT_NAME" \
    ...
    STORAGE_ID="/subscriptions/b4bd79bb-2081-4f1e-8cc4-5c600d3bafc2/resourceGroups/rg-weu-vks-s1-workload-01/providers/Microsoft.Storage/storageAccounts/stvksweus1workload02"
    ...
    echo -e "${GREEN}Associating Storage Account ID=$STORAGE_ID with NSP Profile id=$NSP_PROFILE_ID ...${NC}"

    # Associate the Storage Account with the NSP profile
    az network perimeter association create \
        --name "${NSP_NAME}-stg-assoc" \
        --perimeter-name "$NSP_NAME" \
        --resource-group "$RG_NETWORK" \
